...
module.exports = (_temp = class Compiler extends EventEmitter {
  static get Events() {
    return Events;
  }
  ...
  constructor(options, logger) {
    ...
    _defineProperty(this, "forks", void 0);
    ...
    this.logger = logger;
    this.on(Events.REQUEST_BUNDLE, async ({
      bundleOptions,
      platform,
      filename,
      callback
      ...
      if (!this.forks[platform]) return; // If the fork is compiling the bundle, attach listener to emit `REQUEST_FILE` once
      ...
      if (this.forks[platform].isProcessing) {
        this.forks[platform].once(Events.BUILD_FINISHED, ({
          stats
        }) => {
  ...
  async initFork({
    ...
    fork.on(Events.FILE_NOT_FOUND, ({
      taskId
    }) => {
      const {
        callback,
        awaitingCount
      } = this.tasks.pop(taskId); // If the value is more than 1, it means that we are still awaiting
    ...
    fork.on(Events.BUILD_PROGRESS, payload => {
      this.emit(Events.BUILD_PROGRESS, {
        platform,
        ...payload
      });
    });
  ...
  terminate() {
    Object.keys(this.forks).forEach(platform => this.forks[platform].terminate());
  }
