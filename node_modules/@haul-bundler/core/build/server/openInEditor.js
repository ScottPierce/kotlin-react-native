...
const isKnownEditor = editor => editors.includes(_path.default.basename(editor));
/**
...
function detectEditorType() {
  if (process.env.REACT_EDITOR) {
    const editor = _path.default.basename(process.env.REACT_EDITOR);
...
const hasValidConfig = editorConfig.editor || editorConfig.cmd; // create an editor we'll use to open editor.

const opener = (0, _openInEditor.configure)(editorConfig);
...
function printErrorMessage(runtime) {
  const boldify = text => runtime.logger.enhanceWithModifier('bold', text);

  const greyify = text => runtime.logger.enhanceWithColor('gray', text);
