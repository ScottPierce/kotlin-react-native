...
exports.default = findProvidesModule;
...
const defaultOpts = {
  ...
  blacklist: ['node_modules', '__tests__', '__mocks__', '__fixtures__', 'react-packager', 'androidTest', 'scripts'],
...
const getPlatformFileName = (fileName, platforms) => {
  ...
  const isPlatformExtension = platforms.indexOf(extension) >= 0; // StaticContainer.react would be dropped if we dont count react as a valid extension

  return {
    name: isPlatformExtension ? realName : fileName,
    ignoredPlatformExtension: extension && !isPlatformExtension && extension !== 'react'
  };
};
/**
...
const getProvidedModuleName = fileName => {
  ...
  const segments = fileName.split(_path.default.sep);
  ...
  const walk = dir => {
    ...
    if (stat.isDirectory()) {
      _fs.default.readdirSync(dir).forEach(file => {
        ...
        walk(_path.default.join(dir, file));
      });
    ...
    if (stat.isFile()) {
      ...
      } = getPlatformFileName(jsFileName, options.platforms);
...
//# sourceMappingURL=findProvidesModule.js.map